const utils = {
	 ...
	 *  false:显示
	 */
	request: function(url, postData = {}, method = "POST", contentType = "application/json; charset=UTF-8",
		isDelay, showLoading = false) {
		let loadding = false;
		utils.delayed && uni.hideLoading();
		clearTimeout(utils.delayed);
		utils.delayed = null;
		if (showLoading) { // 只在需要时显示loading
		...
		return new Promise((resolve, reject) => {
			uni.request({
				...
				success: (res) => {
					...
					if (loadding && showLoading) { // 修改这里，使用 showLoading 参数
						uni.hideLoading()
					}
