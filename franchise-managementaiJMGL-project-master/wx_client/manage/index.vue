			...
			<view v-for="(customer, index) in customerList" :key="index" class="customer-card">
				<view class="customer-info">
					...
					<view class="info-left">
						<view class="customer-name">{{ customer.shopName }}</view>
						<view class="customer-phone">{{ customer.shopAddress }}</view>
					</view>

					<!-- 右侧状态 -->
					<view class="info-row">
						<view class="item-status">{{ customer.rate }}</view>
				...
				<view class="action-buttons">
					<button class="btn-detail" @click="viewDetail(customer.shopId)">查看详情</button>
					<button class="btn-follow" @click="followUp(customer.shopId)">上传报告</button>
				</view>
			</view>
...
export default {
	...
	methods: {
		...
		fetchCustomerList() {
			...
			util.request('wx/api/buildList',params, 'GET', 'application/json', false, true)
				.then(res => {
					uni.hideLoading()
					if (res.code === 200) {
		...
		getStatusText(status) {
			...
			return statusTexts[status] || '潜在';
		},

		// 查看客户详情manage
		viewDetail(shopId) {
			uni.navigateTo({
				url: `/manage/detail?id=${shopId}`
			});
		},
