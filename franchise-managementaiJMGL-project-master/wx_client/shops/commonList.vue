<template>
	<view class="container">
		...
		<view class="search-bar">
			<input class="search-input" type="text" v-model="businessName" placeholder="请输入商圈名称" @confirm="search" />
			...
			<view class="shop-list">
				<view class="shop-item" v-for="item in shopList" :key="item.id" @click="goToDetail(item.id)">
					<view class="shop-info">
						<view class="shop-name">商圈名称：{{ item.name }}</view>
						<view class="shop-address">商圈地址：{{ item.address }}</view>
						<view class="shop-address">所属区域：{{ item.region }}</view>
					</view>
				</view>
			</view>
...
export default {
	...
	methods: {
		getShopList(loadMore = false) {
			...
			util.request('wx/api/allDistrictList', {
				pageNum: this.page,
				pageSize: this.limit,
		...
		goToDetail(id) {
			uni.navigateTo({
				url: `/shops/districtDetail?id=${id}`
			});
		}
	}
};
