<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="跟进用户ID" prop="userId">
        <el-input
          v-model="queryParams.userId"
          placeholder="请输入跟进用户ID"
          clearable
    ...
    <el-table v-loading="loading" :data="upList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="跟进用户ID" align="center" prop="userId" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="跟进用户ID" prop="userId">
          <el-input v-model="form.userId" placeholder="请输入跟进用户ID" />
        </el-form-item>
        <el-form-item label="客户ID" prop="clientId">
        ...
        <el-form-item label="跟进详情描述" prop="followUpDescription">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        userId: [
          { required: true, message: "跟进用户ID不能为空", trigger: "blur" }
        ],
        clientId: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除客户跟进记录编号为"' + ids + '"的数据项？').then(function() {
        return delUp(ids)
      }).then(() => {
        this.getList()
